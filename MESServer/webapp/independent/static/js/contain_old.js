require([ './static/utils/js/jquery-3.1.1', './static/utils/js/base/base' ], function($yang, $com) {
	...
	HTML = {
		List_Menu : [
			...
			'<span >{{GroupName}}<b class="caret" style="margin-bottom: 5px"></b></span>',
			'<ul class="femi-neck-float">{{menuHtml}}</ul>',
			'</div>',
			'</li>',
		].join(""),
		List_Item : [
			'<li><div data-src="{{ModuleUrl}}">{{ModuleName}}</div></li>',
		].join("")
	};
		...
		com : {
			...
			render : function(data) {
				...
				$.each(data, function(i, item) {
				    item.menuHtml = $com.util.template(item.ModuleList, HTML.List_Item);
				});
