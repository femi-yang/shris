            ...
            $("body").delegate("#zace-edit-Device-item", "click", function () {
                ...
                $("body").append($com.modal.show(default_value, KEYWORD_Device_item, "修改", function (rst) {
                    ...
                    for (var i = 0; i < BYItem.length; i++) {
                        if (SelectData[0].ID != BYItem[i].ID) {
                            _list.push(BYItem[i]);
                        }
                    }
            ...
            $("body").delegate("#zace-add-Device-item", "click", function () {
                ...
                $("body").append($com.modal.show(DEFAULT_VALUE_Device_item, KEYWORD_Device_item, "新增", function (rst) {
                  ...
                  var  ItemTemp = {
                        ...
                        for (i = 0; i < BYItem.length; i++) {
                            if (BYItem[i].Name == rst.Name && BYItem[i].ModelID == Number(rst.ModelID)) {
                                alert("保养项出现重复！");
                                return false;
                            }
                        }
...
//          //设备保养禁用(项目表)           
        ...
        run: function () {
            $(".Typetable-item  .propertyGrid .femi-property-item input").attr("readonly", "readonly");
        ...
        com: {
            refresh: function () {
                //$(".propertyGrid .femi-property-item input").attr("readonly", "readonly");
//           ModelID:{int},Name:{String},DSType:{int}, Active:{int},StartTime:{DateTime},EndTime:{DateTime}
                model.com.getDeviceMaintainItem({ ModelID: -1, Name: "", DSType: 1, Active: -1, StartTime: "2000-04-26 19:15:19", EndTime: "5029-05-5 10:15:19", BusinessUnitID: 0, BaseID: 0, FactoryID: 0, WorkShopID: 0, LineID: 0 }, function (resItem) {
                    if (!resItem)
                        return;
