           ...
           //设备维修(维修模板表)模糊查询
            ...
            $("body").delegate("#zace-add-Device-Type", "click", function () {
                ...
                $("body").append($com.modal.show(DEFAULT_VALUE_Device_Type, KEYWORD_Device_Type, "新增", function (rst) {
                    ...
                    for (var i = 0; i < WXType.length; i++) {
                        if (WXType[i].Name == rst.Name && WXType[i].ModelID == Number(rst.ModelID)) {
                            alert("维修模板重复！");
                            return false;
                        }
                    }
            ...
            $("body").delegate("#zace-edit-Device-Type", "click", function () {
                ...
                if (SelectData[0].Active == 1) {
                    ...
                    $("body").append($com.modal.show(default_valueThree, KEYWORD_Device_Type, "修改", function (rst) {
                        ...
                        var _list = [];
                        for (var i = 0; i < WXType.length; i++) {
                            if (SelectData[0].ID != WXType[i].ID) {
                                _list.push(WXType[i]);
                            }
                        }
			...
			$("body").delegate("#zace-active-Device-Type", "click", function() {
				...
				for (var j = 0; j < WXType.length; j++) {
				    if (WXType[j].ModelID == Number(SelectData[0].ModelID)) {
				        if (WXType[j].Active == 1) {
...
//          //设备维修禁用(项目表)           
            ...
            $("body").delegate("#zace-type-search-Type", "click", function () {
                var _vdata = { 'header': '设备维修项目', 'href': './device_manage/deviceRepair-item.html', 'id': 'DeviceRepair-item', 'src': './static/images/menu/deviceManage/deviceMaintainItem.png' };
        ...
        run: function () {
            ...
            var wLine = window.parent._Line;
        	   WDataItem=[];
        	   model.com.getDeviceRepairItem({ ModelID: -1, Name: "", DSType: 1, Active: -1, StartTime: "2019-04-26 19:15:19", EndTime: "2029-05-5 10:15:19", BusinessUnitID: 0, BaseID: 0, FactoryID: 0, WorkShopID: 0, LineID: 0 }, function (res1) {
...
//          	ModelID:{int},Name:{String},DSType:{int},Active:{int},StartTime:{DateTime},EndTime:{DateTime}
                model.com.getDeviceRepairType({ ModelID: -1, Name: "", DSType: 1, Active: -1, StartTime: "2019-04-10 15:38:29", EndTime: "2029-05-10 15:38:29", BusinessUnitID: 0, BaseID: 0, FactoryID: 0, WorkShopID: 0, LineID: 0 }, function (resType) {
                    if (!resType)
                        return;
                    if (resType && resType.list) {
                        var Type = $com.util.Clone(resType.list);
                        WXType = $com.util.Clone(resType.list);
//                   var list = model.com.translate(Item);
                ...
                window.parent.WX_Type = 1;
            },
            setItem: function () {
                setTimeout(function () {
                    if (window.parent.WX_Item == 1) {
                        model.com.getDeviceRepairItem({ ModelID: -1, Name: "", DSType: 1, Active: -1, StartTime: "2019-04-26 19:15:19", EndTime: "2029-05-5 10:15:19", BusinessUnitID: 0, BaseID: 0, FactoryID: 0, WorkShopID: 0, LineID: 0 }, function (res1) {
