           ...
           //设备保养(保养模板表)模糊查询
			...
			$("body").delegate("#zace-active-Device-Type", "click", function() {
				...
				for (var j = 0; j < BYType.length; j++) {
				    if (BYType[j].ModelID == Number(SelectData[0].ModelID)) {
...
//          //设备保养禁用(项目表)           
            ...
            $("body").delegate("#zace-type-search-Type", "click", function () {
            	 var _vdata = { 'header': '设备保养项目', 'href': './device_manage/deviceMaintain-item.html', 'id': 'DeviceMaintenance-item', 'src': './static/images/menu/deviceMaintain.png'};
        ...
        com: {
            refresh: function () {
                model.com.getDeviceMaintainType({ModelID:-1,Name:"",DSType:1,Active:-1,StartTime:"2019-04-10 15:38:29",EndTime:"2029-05-10 15:38:29", BusinessUnitID: 0, BaseID: 0, FactoryID: 0, WorkShopID: 0, LineID: 0 }, function (resType) {
                    if (!resType)
                        return;
