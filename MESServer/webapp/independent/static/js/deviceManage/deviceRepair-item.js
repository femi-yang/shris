...
//                  Active:SelectData[0].Active,
                ...
                $("body").append($com.modal.show(default_value, KEYWORD_Device_item, "修改", function (rst) {
                    ...
                    for (var i = 0; i < WXItem.length; i++) {
                        if (SelectData[0].ID != WXItem[i].ID) {
                            _list.push(WXItem[i]);
                        }
                    }
            ...
            $("body").delegate("#zace-add-Device-item", "click", function () {
                ...
                $("body").append($com.modal.show(DEFAULT_VALUE_Device_item, KEYWORD_Device_item, "新增", function (rst) {
                    ...
                    for (i = 0; i < WXItem.length; i++) {
                        if (WXItem[i].Name == rst.Name && WXItem[i].ModelID == Number(rst.ModelID)) {
                            alert("维修项出现重复！");
                            return false;
                        }
                    }
...
//          //设备维修禁用(项目表)           
            ...
            $("body").delegate("#zace-search", "click", function () {
                ...
                $("body").append($com.propertyGrid.show($(".Typetable-item"), default_value, KEYWORD_Device_itemG, TypeSource_Device_item));
        ...
        com: {
            refresh: function () {
                model.com.getDeviceRepairItem({ ModelID: -1, Name: "", DSType: 1, Active: -1, StartTime: "2000-04-26 19:15:19", EndTime: "5029-05-5 10:15:19", BusinessUnitID: 0, BaseID: 0, FactoryID: 0, WorkShopID: 0, LineID: 0 }, function (resItem) {
                    if (!resItem)
                        return;
                    if (resItem && resItem.list) {
                        ...
                        WXItem = $com.util.Clone(resItem.list);
