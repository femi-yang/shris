  ...
  //	    '<td style="min-width: 50px" data-title="PartsRatioList" data-value="{{PartsRatioList}}">{{PartsRatioList}}</td>',
    ...
    model = $com.Model.create({
                ...
                $("body").append($com.modal.show(DEFAULT_VALUE_Device_Regular, KEYWORD_Device_Regular, "新增", function (rst) {
                 ...
                 var   RuleTemp = {
                    ...
                    if (RuleTemp.ModeOptions.length == 0) {
                        alert("维修模式列表不能为空，请选择维修模式列表！");
                        return false;
                    }
                    for (var i = 0; i < WXRule.length; i++) {
                        for (var j = 0; j < WXTyperule.length; j++) {
                            if (WXRule[i].Name == rst.Name && WXTyperule[j].ID == Number(rst.TypeID)) {
                                alert("维修规则重复！");
                                return false;
                            }
                        }
           ...
           //设备维修禁用(规则表)           
            ...
            $("body").delegate("#zace-edit-Device-Regular", "click", function () {
                ...
                $("body").append($com.modal.show(default_valueFive, KEYWORD_Device_Regular, "修改", function (rst) {
                    ...
                    var _list = [];
                    for (var i = 0; i < WXRule.length; i++) {
                        if (SelectData[0].ID != WXRule[i].ID) {
                            _list.push(WXRule[i]);
                        }
                    }
            ...
            $("body").delegate("#zace-type-search-Regular", "click", function () {
                var vdata = { 'header': '设备维修模板', 'href': './device_manage/deviceRepair-type.html', 'id': 'DeviceRepair-type', 'src': './static/images/menu/deviceManage/deviceMaintainType.png' };
            ...
            $("body").delegate("#zace-BY-Device-Regular", "click", function () {
                var vdata = { 'header': '设备维修配置', 'href': './device_manage/deviceMaintain-rule.html', 'id': 'DeviceMaintenance', 'src': './static/images/menu/deviceManage/deviceMaintainSet.png' };
                window.parent.iframeHeaderSet(vdata);
            });
            //点检配置
            ...
            $("body").delegate("#zace-BJBY-Device-Regular", "click", function () {
                var vdata = { 'header': '备件维修配置', 'href': './device_manage/deviceMaintain-rule-bj.html', 'id': 'DeviceMaintenance-bj', 'src': './static/images/menu/deviceManage/deviceMaintainSet.png' };
                window.parent.iframeHeaderSet(vdata);
            });
        ...
        run: function () {
            ...
            WXTyperule = [];
            model.com.getDeviceRepairType({ ModelID: -1, Name: "", DSType: 1, Active: -1, StartTime: "2019-04-10 15:38:29", EndTime: "2029-05-10 15:38:29", BusinessUnitID: 0, BaseID: 0, FactoryID: 0, WorkShopID: 0, LineID: 0 }, function (res1) {
                WXTyperule = res1.list;
        ...
        com: {
            refresh: function () {
                model.com.getDeviceRepairRule({ ModelID: -1, Name: "", TypeID: 0, DSType: 1, Active: -1, BusinessUnitID: 0, BaseID: 0, FactoryID: 0, WorkShopID: 0, LineID: 0 }, function (resRule) {
                    if (!resRule)
                        return;
                    if (resRule && resRule.list) {
                        ...
                        WXRule = $com.util.Clone(resRule.list);
