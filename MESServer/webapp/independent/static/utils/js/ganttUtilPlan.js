        ...
        var install = function (ContainDiv, positionIn, ContainSrocll, date) {
            ...
            canvas.onmousedown = function (evt) {
                ...
                canvas.onmousemove = function (evt) {
                    ...
                    //拖动矩形
                    if (wDownRectResult.wIsLeft == 2 && new Date(new Date(position.Task.data[wDownRectResult.wIndex].startDate).getTime() + (days * 24 * 3600000)) >= new Date(position.series.data[0]) && new Date(new Date(position.Task.data[wDownRectResult.wIndex].startDate).getTime() + ((position.Task.data[wDownRectResult.wIndex].time + days) * 24 * 3600000)) <= new Date(new Date(position.series.data[1]).getTime() + 8 * 3600000) && !position.Task.data[wDownRectResult.wIndex].Locked && position.Task.data[wDownRectResult.wIndex].TaskLineID != 1) {
